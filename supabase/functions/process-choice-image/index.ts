...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { imageUrl, choiceId }: ProcessImageRequest = await req.json();
    console.log(`Processing choice image ${choiceId}: ${imageUrl}`);

    if (!imageUrl || !choiceId) {
      throw new Error("imageUrl and choiceId are required");
    }
    ...
    const processedFileName = `choice-${choiceId}-processed-${Date.now()}.png`;
    ...
    console.log(`Processed choice image uploaded: ${publicUrl}`);

    return new Response(
      JSON.stringify({ 
        success: true,
        processedUrl: publicUrl,
        message: "Choice image processed - white background removed"
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error) {
    console.error("Error processing choice image:", error);
    return new Response(
      JSON.stringify({ 
        success: false, 
