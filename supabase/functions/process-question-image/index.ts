...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Threshold for considering a pixel as "white" or "near-white"
    const WHITE_THRESHOLD = 240; // R, G, B all above this = white
    const LIGHTNESS_THRESHOLD = 0.92; // HSL lightness above this = very light
    ...
    for (let y = 1; y <= image.height; y++) {
      for (let x = 1; x <= image.width; x++) {
        ...
        if (isWhite || isVeryLight) {
          ...
          // For near-white, we could do partial transparency based on how close to white
