-- Update build_daily_plan to respect is_published flag
CREATE OR REPLACE FUNCTION public.build_daily_plan(p_user_id uuid, p_course_id uuid DEFAULT NULL::uuid, p_limit integer DEFAULT 10, p_current_week integer DEFAULT NULL::integer, p_pace_offset integer DEFAULT 1)
...
-- Also update get_recommended_questions to respect is_published
CREATE OR REPLACE FUNCTION public.get_recommended_questions(p_user_id uuid, p_current_week integer DEFAULT 1, p_pace_offset integer DEFAULT 1, p_limit integer DEFAULT 10, p_target_difficulty integer DEFAULT 3, p_course_id uuid DEFAULT NULL::uuid, p_exam_name text DEFAULT NULL::text, p_topic_ids uuid[] DEFAULT NULL::uuid[], p_question_type_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(question_id uuid, prompt text, choices jsonb, correct_answer text, hint text, solution_steps jsonb, difficulty integer, topic_ids text[], question_type_id uuid, source_exam text, due_urgency double precision, knowledge_gap double precision, difficulty_match double precision, score double precision)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  user_mastery AS (
    SELECT 
      tm.topic_id,
      tm.mastery_0_1::double precision as mastery_0_1,
      tm.retention_0_1::double precision as retention_0_1
    FROM topic_mastery tm
    WHERE tm.user_id = p_user_id
  ),
  eligible_questions AS (
    SELECT 
      ...
      (CASE 
        ...
        ELSE 0.0
      END)::double precision as due_urgency,
      (
  ...
  SELECT 
    ...
    eq.knowledge_gap,
    eq.difficulty_match,
    (eq.due_urgency * 0.4 + eq.knowledge_gap * 0.4 + eq.difficulty_match * 0.2)::double precision as score
