-- Update get_recommended_questions to support filtering by course, exam, topics, and question type
CREATE OR REPLACE FUNCTION public.get_recommended_questions(
 ...
 RETURNS TABLE(question_id uuid, prompt text, choices jsonb, correct_answer text, hint text, solution_steps jsonb, difficulty integer, topic_ids text[], question_type_id uuid, source_exam text, due_urgency numeric, knowledge_gap numeric, difficulty_match numeric, score numeric)
 LANGUAGE plpgsql
