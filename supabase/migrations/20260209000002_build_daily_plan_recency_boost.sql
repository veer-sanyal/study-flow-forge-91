...
COMMENT ON FUNCTION public.build_daily_plan IS 'Builds an intentional daily study plan with Review, Current, Bridge, and Stretch questions. Uses FSRS state for prioritization and scheduled_date (exact dates) when available, falling back to scheduled_week. Includes recency boost for topics covered in last 7 days.';
