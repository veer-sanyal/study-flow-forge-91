-- Fix get_recommended_questions: remove reference to t.scheduled_date / t.scheduled_week
--
-- Migration 20260216000001 dropped topics.scheduled_date.
-- The live function was referencing t.scheduled_week (or t.scheduled_date),
-- both of which no longer exist, causing a 400 / code 42703 error.
--
-- Fix: DROP the existing v5 overload (CREATE OR REPLACE cannot change return type)
-- then CREATE fresh without the LEFT JOIN on topics and the dead scheduling WHERE clause.
